...
Foreach($u in $aad_users) {
    ...
    $uguid = $u.ObjectId
        ...
        try {
            ...
            $results = Invoke-RestMethod -Uri ("https://www.facebook.com/scim/v1/Users/?filter=userName%20eq%20%22$uemail%22") -Headers @{Authorization = "Bearer " + $global:token} -UserAgent "WorkplaceScript/UpdateAzureExternalID"
            ...
            If($results.Resources) {
                ...
                $uextid = $user.externalId
                    ...
                    if($askRes.length -eq 0) {
                        ...
                        $user = Invoke-RestMethod -Method PUT -URI ("https://www.facebook.com/scim/v1/Users/" + $uid) -Headers @{Authorization = "Bearer " + $global:token} -ContentType "application/json" -Body $body -UserAgent "WorkplaceScript/UpdateAzureExternalID"
					    Write-Host "$user"
                        #Print OK message
