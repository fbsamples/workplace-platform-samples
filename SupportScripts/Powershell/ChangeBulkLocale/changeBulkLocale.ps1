...
Foreach($u in $global:xslxUsers) {
    ...
    if($uid -And $unewlocale -And $ustatus) {
        ...
        try {
            ...
            $fbuser = Invoke-RestMethod -Method PUT -URI ("https://www.workplace.com/scim/v1/Users/" + $uid) -Headers @{Authorization = "Bearer " + $global:token} -UserAgent "WorkplaceScript/changeBulkLocale" -ContentType "application/json" -Body $body
            If($Interactive.IsPresent) {
