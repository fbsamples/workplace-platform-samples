...
foreach($u in $global:xslxUsers) {
    ...
    $total++
        ...
        try {
            ...
            $results = Invoke-RestMethod -Uri ("https://scim.workplace.com/Users/?filter=userName%20eq%20%22$uemail%22") -Headers @{Authorization = "Bearer " + $global:token} -UserAgent "WorkplaceScript/UpdateBulkAzureManager"
            If($results.Resources) {
                ...
                $unewmanager = Get-AzureADUserManager -ObjectId $uemail | foreach { $_.UserPrincipalName}
                $getnewmanagerid = Invoke-RestMethod -Uri ("https://scim.workplace.com/Users/?filter=userName%20eq%20%22$unewmanager%22") -Headers @{Authorization = "Bearer " + $global:token} -UserAgent "WorkplaceScript/UpdateBulkAzureManager"
                $manager = $getnewmanagerid.Resources[0]
                $unewmanagerid = $manager.id

                #Write-Host "Found user [$ouname] in Workplace and [$unewmanager] as manager in Azure AD. Manager exists with WP ID [$unewmanagerid]"

                $askRes = ""
                If($Interactive.IsPresent) {
                    Write-Host "[$uid/$ouname] will report into-> [$unewmanagerid/$unewmanager]"
                    do {
                    ...
                    Write-Host -NoNewLine "[$uid/$ouname] now reports into-> [$unewmanagerid/$unewmanager]: "
                }
                ...
                if($askRes.length -eq 0) {
                    ...
                    "urn:ietf:params:scim:schemas:extension:enterprise:2.0:User"=@{manager=@{managerId=$unewmanagerid;};};
                    id=$uid;
                    active=$true
                    } | ConvertTo-Json)
                    #Update User via SCIM API
                    $user = Invoke-RestMethod -Method PUT -URI ("https://scim.workplace.com/Users/" + $uid) -Headers @{Authorization = "Bearer " + $global:token} -ContentType "application/json" -Body $body -UserAgent "WorkplaceScript/UpdateBulkAzureManager"
                    #Print OK message
...
Write-Host "- Total User: $total - Updated ($updated), Skipped ($skipped), Errors ($errors)"
Write-Host "---------------------------------------------------------------------------------------------------------"
